...
class AnchorBoxes(Layer):
    ...
    def call(self, x, mask=None):
        ...
        print("size",size,"self.this_scale",self.this_scale)

        print("wh_list",wh_list/size)

        # We need the shape of the input tensor
        # if K.image_dim_ordering() == 'tf':
        if K.image_data_format() == 'channels_last':
            batch_size, feature_map_height, feature_map_width, feature_map_channels = x.get_shape() #._keras_shape
        ...
        # but it can't harm to have this in here for the future
            ...
            batch_size, feature_map_channels, feature_map_height, feature_map_width = x.get_shape() #_keras_shape
        ...
        if (self.this_steps is None):
            step_height = self.img_height / int(feature_map_height)
            step_width = self.img_width / int(feature_map_width)
        else:
        ...
        cy = np.linspace(offset_height * step_height,
                         (offset_height + int(feature_map_height) - 1) * step_height, int(feature_map_height))
        cx = np.linspace(offset_width * step_width,
                         (offset_width + int(feature_map_width) - 1) * step_width, int(feature_map_width))
        ...
        boxes_tensor = np.zeros((int(feature_map_height), int(feature_map_width), self.n_boxes, 4))
        ...
        if self.coords == 'centroids':
            ...
            boxes_tensor = convert_coordinates(boxes_tensor, start_index=0, conversion='corners2minmax',
                                               border_pixels='half')
        boxes_tensor = self.xywh2yxhw(boxes_tensor)
    ...
    def xywh2yxhw(self,boxes_tensor):
        x = boxes_tensor[:,:,:,0]
        y = boxes_tensor[:,:,:,1]
        w = boxes_tensor[:,:,:,2]
        h = boxes_tensor[:,:,:,3]

        new_boxes_tensor = np.zeros((boxes_tensor.shape[0], boxes_tensor.shape[1], boxes_tensor.shape[2], 4))

        new_boxes_tensor[:,:,:,0] = y
        new_boxes_tensor[:,:,:,1] = x
        new_boxes_tensor[:,:,:,2] = h
        new_boxes_tensor[:,:,:,3] = w
