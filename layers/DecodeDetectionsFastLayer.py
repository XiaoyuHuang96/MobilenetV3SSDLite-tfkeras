...
class DecodeDetectionsFast(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        # tf
        # Extract the predicted class IDs as the indices of the highest confidence values.
        # class_ids = tf.expand_dims(tf.to_float(tf.argmax(y_pred[..., :-8], axis=-1)), axis=-1)
        class_ids = tf.expand_dims(tf.cast(tf.argmax(y_pred[..., :-8], axis=-1), dtype=tf.float32), axis=-1)
        
        # Extract the confidences of the maximal classes.
        confidences = tf.reduce_max(y_pred[..., :-8], axis=-1, keepdims=True)
        y_scale = 10.0
        x_scale = 10.0
        height_scale = 5.0
        width_scale = 5.0
